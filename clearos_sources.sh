...
while read -r fsha fname ; do
  if [ ".${fsha}" = ".da39a3ee5e6b4b0d3255bfef95601890afd80709" ]; then
    ...
    if [ ! -e "${fname}" ]; then
      ...
      do
        [ -z "${fsha}" ] && continue
        curl -f "http://koji.clearos.com/kojifiles/source/${pn}/${fname}/${furl}" -o "${fname}" && break
      done
    else
    ...
    if [ "${fsha}" != "${downsum}" ]; then
        rm -f ${fname}
        echo "Failure: ${fname} hash does not match hash from the sources-clearos file" >&2
        exit 1;
    fi
  fi
done < sources-clearos
