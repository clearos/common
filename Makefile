...
if [ -f "$(METADATA)" ]; then echo "../common/Makefile.centos"; elif [ -f "$(APP_SPEC)" ]; then echo "../common/Makefile.app"; elif [ -f "$(SIMPLE_SPEC)" ]; then if [ -f sources ]; then echo "../common/Makefile.fedora"; elif grep -q '^Source0\?:\s*.\+\.tar\.gz' $(SIMPLE_SPEC); then echo "../common/Makefile.simple"; fi; fi
endef
