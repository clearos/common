...
if [ ! -f sources.download ]
then
    echo 'Missing sources.download. Please run from inside a sources git repo' >&2
    exit 1
fi
...
while read -r fsha fname furl; do
  if [ ".${fsha}" = ".da39a3ee5e6b4b0d3255bfef95601890afd80709" ]; then
    ...
    if [ "${fsha}" != "${downsum}" ]; then
        rm -f ${fname}
        echo "Failure: ${fname} hash does not match hash from the sources.download file" >&2
        exit 1;
    fi
  fi
done < sources.download
